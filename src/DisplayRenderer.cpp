...
void DisplayRenderer::begin(DisplaySurface* gfx) {
  gfx_ = gfx;
}

void DisplayRenderer::drawLoading(const String& msg) {
  if (!gfx_) return;
  gfx_->fillScreen(rsvpColorBlack());
  gfx_->setTextColor(rsvpColorWhite());
  gfx_->setTextSize(2);
...
void DisplayRenderer::drawWord(const String& word, AppState state, float pauseAnimProgress) {
  ...
  uint16_t bg = rsvpColorBlack();
  if (state == AppState::PAUSED) {
    uint8_t dim = (uint8_t)(15 * pauseAnimProgress);
    bg = rsvpColor565(dim, dim, dim);
  }
...
void DisplayRenderer::drawOverlayWpm(int wpm) {
  ...
  gfx_->fillRoundRect(x, y, boxW, boxH, 8, rsvpColor565(20, 20, 20));
  gfx_->drawRoundRect(x, y, boxW, boxH, 8, rsvpColor565(120, 120, 120));
  gfx_->setTextColor(rsvpColorWhite());
  gfx_->setTextSize(2);
...
void DisplayRenderer::drawStatus(const String& bookTitle, int wpm, size_t idx, size_t total) {
  if (!gfx_) return;
  gfx_->setTextSize(1);
  gfx_->setTextColor(rsvpColor565(140, 140, 140));
  gfx_->setCursor(6, 6);
...
void DisplayRenderer::drawMenu(const std::vector<String>& items, int selected) {
  if (!gfx_) return;
  gfx_->fillScreen(rsvpColor565(10, 10, 10));
  gfx_->setTextColor(rsvpColorWhite());
  gfx_->setTextSize(2);
  ...
  for (size_t i = 0; i < items.size(); ++i) {
    int y = 54 + i * 30;
    uint16_t c = (i == (size_t)selected) ? rsvpColor565(120, 220, 255) : rsvpColorWhite();
    gfx_->setTextColor(c);
    gfx_->setCursor(28, y);
    gfx_->print(items[i]);
  }
...
void DisplayRenderer::drawChapterList(const std::vector<ChapterAnchor>& chapters, int selected) {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    int y = 54 + i * 22;
    uint16_t c = (idx == selected) ? rsvpColor565(120, 220, 255) : rsvpColorWhite();
    gfx_->setTextColor(c);
    gfx_->setTextSize(1);
    gfx_->setCursor(20, y);
