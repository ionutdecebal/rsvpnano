...
void BoardSupport::powerDownDisplay() {
  setBacklight(0);
  if (tft_) {
    tft_->fillScreen(rsvpColorBlack());
  }
}

bool BoardSupport::initDisplay() {
#if RSVP_HAS_ADAFRUIT_DISPLAY
  spi_.begin(BoardConfig::PIN_TFT_SCLK, BoardConfig::PIN_TFT_MISO, BoardConfig::PIN_TFT_MOSI, BoardConfig::PIN_TFT_CS);
  tft_ = new DisplayPanel(&spi_, BoardConfig::PIN_TFT_CS, BoardConfig::PIN_TFT_DC, BoardConfig::PIN_TFT_RST);
  if (!tft_) {
    return false;
  }
  ...
  tft_->fillScreen(rsvpColorBlack());
#else
  tft_ = new DisplayPanel(BoardConfig::PIN_TFT_CS, BoardConfig::PIN_TFT_DC, BoardConfig::PIN_TFT_RST);
#endif
